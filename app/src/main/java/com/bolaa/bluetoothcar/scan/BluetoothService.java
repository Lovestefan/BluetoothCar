package com.bolaa.bluetoothcar.scan;
...
public class BluetoothService extends Service {
    ...
    private final String UUID="49535343-8841-43F4-A8D4-ECBE34729BB3";
        ...
        bleManager.connectDevice(scanResult, true, new BleGattCallback() {
            ...
            public void onConnectSuccess(final BluetoothGatt gatt, int status) {
               runOnMainThread(new Runnable()
               {
                   @Override
                   public void run() {
                       Toast.makeText(BluetoothService.this,"连接成功",Toast.LENGTH_LONG).show();
                   }
               });
            }
                ...
                runOnMainThread(new Runnable() {
                    ...
                    public void run() {
                        if (mCallback != null) {
                            ...
                            Toast.makeText(BluetoothService.this,"发现服务",Toast.LENGTH_LONG).show();
                            for (final BluetoothGattService service : gatt.getServices()) {
                                if (service.getCharacteristics().size()>0){
                                    for (int i=0;i<service.getCharacteristics().size();i++){
                                        String id = service.getCharacteristics().get(i).getUuid().toString();
                                        Log.e("as",id);
                                        if (service.getCharacteristics().get(i).getUuid().toString().toLowerCase().equals(UUID.toLowerCase())){
                                            bluetoothGattCharacteristic= service.getCharacteristics().get(i);
                                            Log.e("as",bluetoothGattCharacteristic+"");
                                            return;
                                        }
                                    }
                                }
